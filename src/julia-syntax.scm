  ...
  (let ((code '())            ;; statements (emitted in reverse order)
    ...
    (define (pop-exc-expr src-tokens dest-tokens)
      (if (eq? src-tokens dest-tokens)
          (begin
            (display (string "Not emitting pop_exception because token stacks are equal " src-tokens "\n"))
            #f
            )
          (let ((restore-token (let loop ((s src-tokens))
                                 ...
                                 (if (eq? (cdr s) dest-tokens)
                                     (car s)
                                     (loop (cdr s))))))
            (display (string "Emitting pop_exception. token stacks: " src-tokens " -> " dest-tokens "\n"))
    ...
    (define (emit-break labl)
      (display (string "emit-break " labl "  .... (catch-token stack is " catch-token-stack ")\n"))
      (let ((lvl (caddr labl))
               ...
               (let* ((v1  (compile (cadr e) break-labels value #f)) ;; emit try block code
                 ...
                 (if finally
                     ...
                     (begin (set! catch-token-stack (cons handler-token catch-token-stack))
                            (display (string "Set token stack to " catch-token-stack "\n"))
                            ...
                            (display (string "Reset token stack to " catch-token-stack "\n"))
                            ))
                 val)))
